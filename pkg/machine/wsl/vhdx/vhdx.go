...
// Create vhdx
func Create(path machineDefine.VMFile, maxSizeInBytes uint64) error {
	...
	handle, err := vhd.CreateVirtualDisk(path.GetPath(), vhd.VirtualDiskAccessNone, vhd.CreateVirtualDiskFlagSparseFile|createVirtualDiskFlagSupportSparseFileAnyFs, &params)
	if err != nil {
