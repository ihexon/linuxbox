...
// compressed files (.gz, .xz...), archiver files (.tar, .zip...), and
// compressed archiver files (tar.gz, tar.bz2...). The returned Format
...
func Identify(filename string, stream io.Reader) (Format, io.Reader, error) {
	...
	for name, format := range formats {
		...
		// so we can see if it contains an archiver within
...
// CompressedArchive combines a compression format on top of an archiver
// format (e.g. "tar.gz") and provides both functionalities in a single
// type. It ensures that archiver functions are wrapped by compressors and
...
//
// As this type is intended to compose compression and archiver formats,
...
// Match matches if the input matches both the compression and archiver format.
func (caf CompressedArchive) Match(filename string, stream io.Reader) (MatchResult, error) {
	...
	if caf.Compression != nil {
		...
		// attempt to match the archiver by reading the stream
...
// Archive adds files to the output archiver while compressing the result.
...
// ArchiveAsync adds files to the output archiver while compressing the result asynchronously.
func (caf CompressedArchive) ArchiveAsync(ctx context.Context, output io.Writer, jobs <-chan ArchiveAsyncJob) error {
	...
	if !ok {
		return fmt.Errorf("%s archiver does not support async writing", caf.Name())
	}
	if caf.Compression != nil {
		wc, err := caf.Compression.OpenWriter(output)
		if err != nil {
...
// Extract reads files out of an archiver while decompressing the results.
