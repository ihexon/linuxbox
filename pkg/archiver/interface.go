...
// Format represents either an archiver or compression format.
...
// Archival is an archival format with both archiver and extract methods.
...
// Archiver can create a new archiver.
type Archiver interface {
	// Archive writes an archiver file to output with the given files.
	//
...
type ArchiverAsync interface {
	...
	// the files for the archiver. Close the jobs channel after
...
// Extractor can extract files from an archiver.
...
// Inserter can insert files into an existing archiver.
// EXPERIMENTAL: This API is subject to change.
type Inserter interface {
	// Insert inserts the files into archiver.
	//
	// Context cancellation must be honored.
